...
.required{color:#F00;}
	...
	document.getElementById('save').onclick=function()  //called when save button is clicked
	{
...
/*		var phoneno = /^\d{10}$/;
      if(contactNo.value.match(phoneno))
    		...
    		if(userName.length<1 || email.length<1 || address.length<1 || gender.length<1 || pass.length<1 || icNo.length<1 || yoe.length<1 || acadQ.length<1 || manager.length<1 || stype.length<1 || department.length<1)
		{
			...
			data: 
			{  
			   'action' :'checkemployee',   //call login.php and send this add to add employee
			   'email' : email,
			...
			success: function (html) 
			{
			  ...
			  if(html.includes("exists"))  {
		    	alert("The employee user already exists");
	...
	//	alert(userName);
		$.ajax({
			...
			data: 
			{  
			   'action' :'addemployee',   //call login.php and send this add to add employee
			   'username' :userName,
			   'password' :pass,
			   'fname' :firstName,
			   'lname': lastName,
